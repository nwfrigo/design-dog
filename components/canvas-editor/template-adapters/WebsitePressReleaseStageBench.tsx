...
/**
 * Stage & Bench adapter for website-press-release (factory-driven).
 *
 * 800×450 press-release card. Two-column layout: left content stack
 * (logo + solutionPill anchor, then eyebrow / headline / subhead /
 * body / cta), right image. Stage bar: theme + content-stack alignment.
 */

const IMAGE_PLACEHOLDER = '/placeholder-mountain.jpg'

export const WebsitePressReleaseStageBench =
  defineStageBenchAdapter<WebsitePressReleaseBlockId>({
    ...
    contentStack: { templateKey: 'website-press-release', maxGap: 96 },
    useStoreBindings: () => {
      ...
      const gaps = useStore((s) => s.templateGaps['website-press-release'] ?? {})
      ...
      const raw = thumbnailImageSettings['website-press-release']
      ...
      return {
        ...
        image: {
          ...
          setSettings: (next) => setThumbnailImageSettings('website-press-release', next),
          frameWidth: 338,
          frameHeight: 450,
        },
        category: {
        ...
        contentStack: {
          ...
          setStackAlign,
          gaps,
          setGap: (key, value) => setTemplateGap('website-press-release', key, value),
        },
        extras: { theme, logoColor, grayscale, solution },
      }
    },
    renderTemplate: (ctx) => {
      const theme = ctx.extras.theme as TemplateTheme
      const logoColor = ctx.extras.logoColor as 'black' | 'orange' | 'white'
      ...
      return (
        <WebsitePressRelease
          ...
          stackAlign={ctx.stackAlign as StackAlign}
          gaps={ctx.gaps}
