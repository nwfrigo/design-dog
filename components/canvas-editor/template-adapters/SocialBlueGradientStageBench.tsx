...
import type { ColorStyle, TextAlignment, HeadingSize, CtaStyle } from '@/types'
...
/**
 * Stage & Bench adapter for social-blue-gradient (factory-driven).
 *
 * Structural sibling of social-dark-gradient. Same 6 editable blocks +
 * logo topAnchor, same ContentStack-based layout. Differs by pinning
 * the logo to white (no logoColor toggle) and swapping the swatches
 * to blue-gradient assets.
 */
...
export const SocialBlueGradientStageBench = defineStageBenchAdapter<SocialBlueGradientBlockId>({
  ...
  contentStack: { templateKey: 'social-blue-gradient', maxGap: 96 },
  useStoreBindings: () => {
    ...
    const gaps = useStore((s) => s.templateGaps['social-blue-gradient'] ?? {})
    ...
    return {
      ...
      contentStack: {
        ...
        setStackAlign,
        gaps,
        setGap: (key, value) => setTemplateGap('social-blue-gradient', key, value),
      },
      extras: { colorStyle, headingSize, ctaStyle, alignment },
    }
  },
  renderTemplate: (ctx) => {
