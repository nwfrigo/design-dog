...
import {
  getWebsiteThumbnailSlots,
  getWebsiteThumbnailSizes,
  getWebsiteThumbnailContents,
} from '../template-configs/website-thumbnail'
import {
  WebsiteThumbnail,
  type WebsiteThumbnailBlockId,
} from '../../templates/WebsiteThumbnail'
import type { StageBenchEditorProps } from '../StageBenchEditor'

/**
 * Stage & Bench adapter for website-thumbnail.
 *
 * Press-release-shaped: ContentStack with logo+pill topAnchor on the
 * left, image (rounded rect 320×386) on the right via the universal
 * `thumbnailImageSettings['website-thumbnail']` model. Stage-bar adds a
 * 2-state `variant` (image/none) Toggle that swaps the right column
 * between an editable image frame and pure text.
 */
...
export function WebsiteThumbnailStageBench(props: StageBenchEditorProps) {
  const {
    selectedAssets,
    currentAssetIndex,
    isExporting,
  ...
  const ebookVariant = useStore((s) => s.ebookVariant)
  const setEbookVariant = useStore((s) => s.setEbookVariant)
  ...
  const websiteThumbnailGaps = useStore((s) => s.templateGaps['website-thumbnail'] ?? {})
  ...
  const rawImageSettings = thumbnailImageSettings['website-thumbnail']
  ...
  const slots = getWebsiteThumbnailSlots({
  ...
  const slotConfig: Record<WebsiteThumbnailBlockId, { storeKey: string; kind: 'text' | 'cta' | 'image' | 'pill' }> = {
        ...
        >
          ...
          >
            <CategoryRegistryProvider
              categories={[
                {
                  path: 'website-thumbnail.solutionPill',
            ...
            >
              ...
              >
                <WebsiteThumbnail
                  ...
                  imageUrl={thumbnailImageUrl ?? '/assets/images/safer_is_stronger_sample_page.png'}
                  ...
                  renderSpacerBetween={(key, value) => (
                    ...
                    >
                      <SpacingHandle
                        spacing={value}
                        onChange={(next) => setTemplateGap('website-thumbnail', key, next)}
                        scale={1}
                  ...
                  renderBlock={(blockId, content) => {
                    ...
                    const slotPath = `website-thumbnail.${blockId}`
                  ...
                  renderInlineEditor={(blockId, defaultInner) => {
                    const path = `website-thumbnail.${blockId}`
            ...
            <ImageEditorModal
              ...
              onSettingsChange={(next) => {
                setThumbnailImageSettings('website-thumbnail', {
