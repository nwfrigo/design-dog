...
/**
 * Stage & Bench config for social-image-meddbase.
 *
 * Twin of social-image (same slot inventory: solutionPill / subhead /
 * metadata / cta — headline mandatory, image fixed-presence). Differs at
 * the template render layer (MeddbaseLogo + non-themed surface colors),
 * not in slot shape.
 */
...
export function getSocialImageMeddbaseSlots(s: SlotsParams): SlotVisibility[] {
  return [
...
export function getSocialImageMeddbaseSizes(s: SizesParams): SlotSize[] {
  return [
    {
      path: 'social-image-meddbase.headline',
    ...
    {
      path: 'social-image-meddbase.subhead',
...
export function getSocialImageMeddbaseContents(s: ContentsParams): SlotContent[] {
  return [
    { path: 'social-image-meddbase.headline', format: 'html',  value: s.headlineHtml, set: s.setHeadlineHtml },
    { path: 'social-image-meddbase.subhead',  format: 'html',  value: s.subheadHtml,  set: s.setSubheadHtml },
    { path: 'social-image-meddbase.metadata', format: 'plain', value: s.metadata,     set: s.setMetadata },
    { path: 'social-image-meddbase.cta',      format: 'plain', value: s.ctaText,      set: s.setCtaText },
  ]
}
