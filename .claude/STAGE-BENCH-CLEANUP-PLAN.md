...
| 1 phase A — Factory + 3 pilots | ✅ Done | `949903c`, `93ba8ae`, `675a8ca` | `defineStageBenchAdapter` lands. Pilots ported: social-ehs-accelerate (−54%), social-image (−45%), email-cority-connect-2026 (−53%). Validation pass added color-N selector kinds. Pilot follow-up fixed category dropdown wrap order, 'Solution' → 'Category' label, CC2026 font-size default. Multi-image (EmailSpeakers) deferred via `renderTemplate` escape. |
| 3 §3.2 + §3.3 — Conventions | ✅ Done | `7b85321`, `e87d255` | Five subsections under §8 of STAGE-AND-BENCH.md (factory pointer, visibility-flag naming, bench-toggle-ability, block existence ≠ content + canonical placeholders, styling-wrappers-outside-wrapInline). |
| WYSIWYG inline-edit sweep | ✅ Out-of-plan | `45c5ede` | Styled wrappers moved outside `wrapInline` across 12 templates so editor inherits typography. CTA arrows + per-template flavors preserved. |
| Block existence ⊥ content presence | ✅ Out-of-plan | `611c819`, `76596af`, `1b1408d`, `1bbebaf` | `CONTENT-BLOCK-AUDIT.md` seeded as the spec, then swept across catalog: dropped every `&& !!X` / `&& hasX` clause; canonical placeholders ('Headline', 'Body copy goes here.', 'Call to Action', etc.) live in template fallbacks; templates own the placeholder. |
| Default visibility centralization | ✅ Out-of-plan | `cfafeeb` | `lib/template-defaults.ts` is the single source of truth: canonical block-kind defaults + per-template overrides + `BRANDED_SEEDS` (NEWS / EBOOK / REPORT / Webinar / LIVE EVENT / Product Release). Store init + TemplateTile both consume. |
| Net-less reconciliation | ✅ Out-of-plan | `f656524`, `1412dba`, `47c618c` | Killed every scattered placeholder source: `PREVIEW_PLACEHOLDERS` deleted from 24 adapters, store seeds emptied, EditorScreen + AssetSidebar legacy fallbacks dropped, registry parser/assembleProps defaults aligned, `draft-storage` save was silently re-injecting 'Responsive' — fixed via `?? ''` everywhere. `goToEditorWithTemplate` now spreads the FULL `...defaults` so editor entry matches thumbnail. |
| Substrate UX: drag text-selection + responsive stage | ✅ Out-of-plan | `11dff99`, `2f8e11a`, `3376f87`, `69b1dba` | DndProvider sets `data-dnd-dragging` on body + clears Selection at activation → no more text highlighting bleed-through. ScaledStage wraps the stage with `transform: scale()` driven by ResizeObserver on `<main>`'s available width → social templates fit on 14" laptops. |
| Round of template/registry fixes | ✅ Out-of-plan | `bfee369` | Metadata → Small Caption label (5 places); FloatingBanner desktop+mobile `showCta` gating + drag-to-bench unblock; grid inline edit on 4 grid templates; Report + Thumbnail variant → `kind: 'layout-2'` (new 2-cell SelectorPrimitive variant reusing image/text icons); SocialDarkGradient logo-color toggle → `kind: 'color-2'` swatches. |
| **Visual smoke test of 3 pilots** | ✅ Effectively done | — | Visually verified through the iterations above. Pilots match thumbnails; inline edit / bench drag / stage-bar selectors / image modal all confirmed working through the UX hygiene sweeps. |
| 2 — Auto-registration (pilots) | ✅ Done | `5502e13` | `lib/stage-bench-registry.ts` is the single source of truth: bundles `renderProps` + `renderSchema` + `queueTextFields` + `exportBuilder` + `templateId` per template. `template-registry.tsx`, `export-params.ts`, and `migrated-templates.ts` all consume it. 3 pilots ported. Remaining surface: client-side `TEMPLATE_ADAPTERS` map in `StageBenchEditor.tsx` still hardcodes 27 entries — naturally folds in during Task 1 phase B as each adapter ports onto the factory + registration. |
| 1 phase B — Mass-port 24 adapters | ✅ Done | `c2042e9`, `4e5816f`, `03f7162`, `210c569`, `075b0aa`, `8098fd3`, `b7b0ba1`, `5aef4f7`, `bf663d0`, `b13db14` | All 24 legacy adapters now consume `defineStageBenchAdapter` (or, for `email-speakers`, register only the metadata + keep the 568-LOC hand-rolled adapter via the §1.4 escape path). Bundle order: newsletter trio → social gradient pair → social/email image pair → email dark gradient → banner/signature batch (CCE + EHS + FloatingBanner desktop/mobile) → product/press release → website thumbnail/report/webinar → 4 grids → email-speakers. Factory grew a `layout-2` selector kind for the thumbnail/report image-vs-text toggle. |
| 2 — Adapter fold into registry | ✅ Done | `227b83f` | Each Registration now carries the React Adapter too; `StageBenchEditor.tsx` drops its 27-entry hardcoded map in favor of `getStageBenchAdapter(id)`. Adding a new S&B template is now one *Registration.ts file — no separate touches to template-registry, export-params, migrated-templates, or StageBenchEditor. |
| 3 §3.1 — Grid off-by-one rename | ⏸ Deferred | — | Logged in `SUBSTRATE-DEBT.md`. The straightforward `showRow3` → `showGridDetail2` rename collides with EmailGrid's existing `showGridDetail2` field — needs a different target name. The 4 grid templates ported with the legacy names; ledger entry has the trigger condition. |
| 6 — Contribution playbook | Not started | — | Rewrite STAGE-BENCH-MIGRATION.md once Nick reviews the factory + central registry. |
