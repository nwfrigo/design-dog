...
- [template] All template components MUST have `'use client'` as the first line. The `[slug]` render page passes `Component={entry.component}` as a prop to `GenericRenderContent` which is a Client Component. Next.js cannot serialize a Server Component function across that boundary — it throws "Functions cannot be passed directly to Client Components". Every template in the codebase has `'use client'`; always add it when creating a new template component.

- [export] Never use `next/image` (`<Image>`) in template components. All template components must use plain `<img>` tags. `next/image` routes through Vercel's image optimization service (`/_next/image?url=...`), which adds a network request that Puppeteer's `networkidle2` waits on — causing navigation timeouts on Vercel. Every other template in the codebase uses `<img>`; this is the required pattern.
