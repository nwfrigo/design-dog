...
### Logging deferred substrate work

When substrate-level work is deferred (a feature stubbed out, a primitive intentionally limited, a migration scoped down), add an entry to `SUBSTRATE-DEBT.md` *before the commit lands*. Don't rely on commit messages or chat history to remember. Each entry needs a **Trigger condition** — the moment paying it down becomes urgent — so future-you can decide whether the debt is still latent or now due.
