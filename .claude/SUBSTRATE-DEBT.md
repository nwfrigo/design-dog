...
**What:** `kind:'image'` slots have no hide affordance — images don't drag to the bench (factory suppresses drag for images) and `EditbarImage` has no EyeOff button. So an image is either always-on or absent; there's no user toggle. `executive-overview` works around this by making its 3 images (partner logo, hero, contact avatar) always-on `benchable:false` with empty-state placeholders, and the 5 EHS+ Accelerate partner slots (2026-09) follow the same pattern — so six templates now show an always-on partner placeholder in the editor. Exports are safe: `PartnerLogo` renders nothing when unset + non-interactive; the cost is editor-only placeholder clutter for Cority-only assets.
...
**Cost to ignore:** Templates can't offer optional images; every partner-slot template shows a permanent placeholder box in the editor even when no partner exists for the asset.
